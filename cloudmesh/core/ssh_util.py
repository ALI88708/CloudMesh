...
def build_ssh_cmd(host, user, key, cmd, extra_flags=None):
...
def run_ssh(host, user, key, cmd, timeout=30, extra_flags=None):
...
def run_ssh_with_stdin(host, user, key, stdin_data, cmd, timeout=30):
    ssh_cmd = build_ssh_cmd(host, user, key, cmd)
    try:
